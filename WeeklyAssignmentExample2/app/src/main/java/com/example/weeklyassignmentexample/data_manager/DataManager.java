...
import com.example.weeklyassignmentexample.RetrofitCallbacks.ApiInterface;
import com.example.weeklyassignmentexample.RetrofitCallbacks.RetrofitClientInstance;
...
//                        dbHelper.insertDataIntoDB(database, response.body().getData(), response.body().getPage());
    ...
    public void getUsersListFromDB(int page_no, DBHelper dbHelper, SQLiteDatabase database) {
        ...
        List<Datum> usersModelList = new ArrayList<>();
        if (databasePageCount >= page_no) {
            Datum usersModel;
            while (!cursor.isLast()) {
                if (cursor.getInt(4) == page_no) {
                    usersModel = new Datum();
    ...
    public void shareUserRequest(Context context, Datum usersModel) {
